...
export async function generateMetadata({ params }: Props) {
  ...
  const { data: article } = await supabase
    ...
    .lte('published_at', nowIso) // HANYA BISA DIAKSES JIKA SUDAH LEWAT TANGGAL RILIS
    .single();
...
export default async function BlogDetailPage({ params }: Props) {
  ...
  const { data: article, error } = await supabase
    ...
    .eq('slug', slug)
    .or('is_published.eq.true,is_published.is.null')
    .lte('published_at', nowIso) // LINDUNGI ARTIKEL MASA DEPAN DARI AKSES LANGSUNG VIA URL
    .single();
  ...
  const cleanedContent = article.content ? article.content.replace(/^#\s+[^\n]+\n?/, '') : '';
    ...
    <article className="max-w-4xl mx-auto px-4 py-12">
      ...
      <header className="mb-8 space-y-4">
        ...
        <p className="text-gray-500 text-sm">
          Estimasi waktu baca: {article.reading_time_minutes || 5} menit
        </p>
      </header>
